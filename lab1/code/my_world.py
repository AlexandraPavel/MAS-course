...
class MyPredatorWithCommunication(MyPredator):
    ...
    def response(self, perceptions: MyAgentPerception) -> SocialAction:
        ...
        for (pred_id, _) in perceptions.nearby_predators:
            if pred_id not in self.knowned_preds:
                self.knowned_preds.append(pred_id)

        agent_pos = perceptions.agent_position
        nearest_pray_pos = None
        nearest_dist = 10000

        for (_, pray_pos) in perceptions.nearby_prey:
            messages_to_send.append(pray_pos)
            distance = agent_pos.get_distance_to(pray_pos)

            if nearest_dist > distance:
                nearest_dist = distance
                nearest_pray_pos = pray_pos

        greedy_action = None

        if nearest_pray_pos is not None:
            relative_orientation = agent_pos.get_simple_relative_orientation(nearest_pray_pos)

            if relative_orientation == GridRelativeOrientation.FRONT:
                greedy_action = MyAction.NORTH

            elif relative_orientation == GridRelativeOrientation.BACK:
                greedy_action = MyAction.SOUTH

            elif relative_orientation == GridRelativeOrientation.RIGHT:
                greedy_action = MyAction.EAST

            elif relative_orientation == GridRelativeOrientation.LEFT:
                greedy_action = MyAction.WEST
            
            elif relative_orientation == GridRelativeOrientation.FRONT_LEFT:
                greedy_action = random.choice([MyAction.NORTH, MyAction.WEST])
            
            elif relative_orientation == GridRelativeOrientation.FRONT_RIGHT:
                greedy_action = random.choice([MyAction.NORTH, MyAction.EAST])
            
            elif relative_orientation == GridRelativeOrientation.BACK_LEFT:
                greedy_action = random.choice([MyAction.SOUTH, MyAction.WEST])
            
            elif relative_orientation == GridRelativeOrientation.BACK_RIGHT:
                greedy_action = random.choice([MyAction.SOUTH, MyAction.EAST])

        if greedy_action is not None:
            if messages_to_send is []:
                return greedy_action
            
            social_action = SocialAction(greedy_action)
            for k_pred in self.knowned_preds:
                social_action.add_outgoing_message(self.id, k_pred, messages_to_send)
        ...
        if nearest_pray_pos is not None and nearest_dist < self.max_fail_dist:
            relative_orientation = agent_pos.get_simple_relative_orientation(nearest_pray_pos)
        ...
        for predator_data in self._predator_agents:
            if not predator_data in agent_actions:
                ...
                if isinstance(predator_action, SocialAction):
                    msgs = predator_action.outgoing_messages
                    self.message_box += msgs
