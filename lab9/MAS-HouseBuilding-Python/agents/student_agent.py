...
class MyCompanyAgent(CompanyAgent):
    ...
    def decide_bid(self, auction_item: str, auction_round: int, item_budget: float) -> bool:
        ...
        if (len(self.contracts) == 0 and self.specialties[auction_item] * 0.85 <= item_budget) or self.specialties[auction_item] <= item_budget:
            return True
        else:
            return False
    ...
    def respond_to_offer(self, initiator_msg: NegotiationMessage) -> float:
        ...
        if initiator_msg.negotiation_item in self.participants_per_items:
            return self.specialties[initiator_msg.negotiation_item] *  0.96 * pow(0.95 - self.participants_per_items[initiator_msg.negotiation_item] * 0.01, initiator_msg.round)
        
        return self.specialties[initiator_msg.negotiation_item] *  pow(1.02, initiator_msg.round)
